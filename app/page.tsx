...
export default function ChatPage() {
  ...
  const clearChat = () => {
    ...
    // Create a completely new conversation with new ID
    const newConversationId = `conv_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    setMessages([])
    setShowQuickLinks(true)
    setCurrentConversationId(newConversationId)

    console.log('[Chat] Started new conversation:', newConversationId)
  }
          ...
          <div className="md:hidden flex justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              {messages.length > 0 && (
                <button
                  onClick={clearChat}
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium text-xs"
      ...
      <div className="flex-1 overflow-y-auto flex flex-col w-full" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        <div className="flex-shrink-0 bg-white border-t-2 border-gray-200 p-4 shadow-2xl safe-area-bottom">
          <div className="max-w-5xl mx-auto space-y-3">
            ...
            {/* Simple Voice Input (Web Speech API only) */}
            <div className="mt-4 flex items-center justify-center gap-2 text-sm text-gray-600">
              <span>Quick voice input:</span>
              <SimpleVoiceButton
              ...
              />
              <span className="text-xs text-gray-500">(Tap blue mic to speak)</span>
            </div>
